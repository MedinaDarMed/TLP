...
class Juego:
    def __init__(self, datos_juego):
        ...
        if self.tipo_juego == 'TETRIS':
            ...
            # FIX: TETRIS necesita self.level para mostrar_game_over()
            self.level = 'TETRIS'
        ...
        # (Actividad 5-A): Inicialización del juego TANKS 
        if self.tipo_juego == 'TANKS':
            ...
            # Jugador
            # Buscar la shape con ENTITY_ROLE = PLAYER
            ...
            self.player_speed_counter = 0   # contador de ticks para throttling

            # Enemigos
            # Cada enemigo es un dict con: shape, pos, hp, hp_max, speed,
            # fire_rate, fire_counter, color, cbh, is_boss
            #
            # CORRECCIÓN (Punto B): el FINAL_BOSS NO se agrega aquí.
            # Aparece dinámicamente al ejecutarse ON_TARGET_SCORE cuando
            # el jugador alcanza boss_score_threshold puntos.
            # Solo los ENEMY normales se pre-crean (pos [0,0] se asigna en ON_START).
            self.enemies = []
            for shape_name, role in roles.items():
                if role == 'ENEMY':           # ← solo ENEMY, no FINAL_BOSS
                    self.enemies.append({
                        'shape':        shape_name,
                        'pos':          [0, 0],       # se sobreescribe en ON START
                        ...
                        'is_boss':      False,
                        'alive':        False,   # se activa al hacer SPAWN en ON_START
                    })

            # Proyectiles
            # Lista de balas: {'pos': [x,y], 'dir': dir, 'owner': 'player'|'enemy'}
            self.bullets = []

            # ── Actividad 6: Muros destructibles ──────────────────────────────
            # Cada muro: {'pos':[x,y], 'hp':int, 'hp_max':int, 'color':str, 'cbh':dict}
            # Se crean en ejecutar_evento() cuando role == 'WALL'.
            # Los proyectiles verifican colisión contra self.walls en tanks_tick_bullets().
            self.walls = []

            # Martillo (powerup)
            self.hammer_pos     = None   # None = no está en el tablero
            self.hammer_timer   = 0      # cuenta ticks para aparición aleatoria
            self.hammer_heal    = self.datos_juego.get('powerups', {}).get(
                                    'HAMMER_PU', {}).get('chance', 15)
            # Los puntos de vida que restaura el martillo (fijo: 25)
            self.hammer_hp_restore = 25

            # Estado de victoria / oleadas
            self.tanks_game_over = False
            self.tanks_player_win = False   # ya no se usa, pero se mantiene por retrocompat
            self.tanks_wave = 1             # oleada actual
            self.tanks_kill_count = 0       # enemigos eliminados en total

            # ── Actividad 5-B: Estado de la fase del Final Boss ──
            # boss_score_threshold: puntos necesarios para disparar la transicion.
            # Se lee del JSON (campo 'boss_score_threshold') con 500 como default.
            self.boss_score_threshold = self.datos_juego.get('boss_score_threshold', 500)
            # boss_phase_active: True cuando ya se disparo el evento ON_TARGET_SCORE.
            self.boss_phase_active = False
            # boss_defeated: True cuando el boss (FINAL_BOSS) muere.
            self.boss_defeated = False
            # tanks_wave_spawned: True en cuanto ON_START activa al menos un enemigo.
            # Evita que tanks_check_win dispare respawn antes de que el juego empiece.
            self.tanks_wave_spawned = False
            # Plantillas para respawnear enemigos (copia de la config original)
            # CORRECCIÓN: solo se guardan plantillas de ENEMY normales.
            # El boss NO está en self.enemies aún (se agrega en ON_TARGET_SCORE),
            # así que este bucle ya lo excluye naturalmente. Se documenta
            # explícitamente para que no se cambie por error.
            self.tanks_enemy_templates = []
            for e in self.enemies:
                if not e['is_boss']:     # ← guardia explícita: nunca plantilla del boss
            ...
            # Actualizar labels del panel (ya creado arriba en bloque GUI) con valores reales
    ...
    def game_loop(self):
        ...
        if self.tipo_juego == 'TANKS':
            ...
            self.tanks_check_boss_transition()   # Actividad 5-B: monitorear puntuacion para el boss
            self.tanks_check_win()
            self.dibujar()
            self.root.after(80, self.game_loop)   # ~12 fps para TANKS
            return
    ...
    def manejar_input_gui(self, event):
        ...
        if self.tipo_juego == 'TANKS':
            ...
            if event.keysym in mapa:
                ...
                # Actividad 6: disparar en la dirección actual del jugador
    ...
    def dibujar(self):
        ...
        # TANKS usa sus propios labels (lbl_score, lbl_player_hp) — no tocar label_score
        if self.tipo_juego != 'TANKS':
            if self.es_remake:
        ...
        # TANKS: dibujo del estado del juego
        elif self.tipo_juego == 'TANKS':
            self.canvas.delete('all')
            ts = self.taman_celda

            # ── Fondo: campo de batalla con textura de tierra ──
            for y in range(self.alto):
                for x in range(self.ancho):
                    # Alternancia sutil de tonos tierra/hierba
                    base = '#111820' if (x + y) % 2 == 0 else '#0D141C'
                    x1c, y1c = x * ts, y * ts
                    self.canvas.create_rectangle(x1c, y1c, x1c + ts, y1c + ts,
                                                 fill=base, outline='')

            # Líneas de cuadrícula muy sutiles
            for x in range(0, self.ancho * ts, ts):
                self.canvas.create_line(x, 0, x, self.alto * ts,
                                        fill='#1A2230', width=1)
            for y in range(0, self.alto * ts, ts):
                self.canvas.create_line(0, y, self.ancho * ts, y,
                                        fill='#1A2230', width=1)

            # Actividad 5-B: Banner de alerta cuando la fase del boss esta activa.
            # Se dibuja un borde rojo/magenta parpadeante alrededor del canvas
            # para indicar visualmente que el jefe ha entrado en juego.
            if getattr(self, 'boss_phase_active', False):
                # Borde de alerta magenta (4 px)
                self.canvas.create_rectangle(
                    0, 0, self.ancho_canvas - 1, self.alto_canvas - 1,
                    outline='#FF00FF', width=4
                )
                # Texto de advertencia centrado en la parte superior
                self.canvas.create_text(
                    self.ancho_canvas // 2, 20,
                    text=u'!! FINAL BOSS !!',
                    fill='#FF00FF',
                    font=('Courier', max(10, ts // 2), 'bold')
                )

            # ── Banner de oleada (esquina superior derecha del canvas) ──
            oleada_txt = u'OLEADA  ' + str(self.tanks_wave)
            self.canvas.create_text(
                self.ancho_canvas - 8, 8,
                text=oleada_txt, anchor='ne',
                fill='#FFDD44', font=('Courier', max(8, ts // 4), 'bold')
            )
            ...
            if self.hammer_pos:
                ...
                r_pu = ts // 3
                self.canvas.create_oval(
                    hcx - r_pu, hcy - r_pu, hcx + r_pu, hcy + r_pu,
                    fill='#FFDD00', outline='#FF8800', width=2
                )
                self.canvas.create_text(hcx, hcy, text=u'\u2665',
                                        fill='#AA4400',
                                        font=('Courier', max(8, ts // 3), 'bold'))
            ...
            for bala in self.bullets:
                ...
                # Halo de luz
                self.canvas.create_oval(
                    bx_px - r_bala - 2, by_px - r_bala - 2,
                    bx_px + r_bala + 2, by_px + r_bala + 2,
                    fill=glow, outline=''
                )
                # Núcleo
                self.canvas.create_oval(
                    bx_px - r_bala, by_px - r_bala,
                    bx_px + r_bala, by_px + r_bala,
                    fill=col_bala, outline=''
                )
            ...
            for muro in self.walls:
                ...
                mx_px = mx * ts
                my_px = my * ts
                # Color del muro según vida (mismo sistema que tanques)
                ...
                if cbh_m:
                    ...
                    for umbral_m, c_m in cbh_m.items():
                        ...
                        if pct_m <= u_m and u_m > mejor:
                            mejor = u_m
                            col_m = c_m
                # Cuerpo del muro (2x2 celdas, mismo tamaño que la shape)
                self.canvas.create_rectangle(
                    mx_px, my_px, mx_px + ts * 2, my_px + ts * 2,
                    fill=col_m, outline='#444433', width=1
                )
                # Detalle: líneas de ladrillos
                self.canvas.create_line(mx_px, my_px + ts,
                                        mx_px + ts * 2, my_px + ts,
                                        fill='#444433', width=1)
                self.canvas.create_line(mx_px + ts, my_px,
                                        mx_px + ts, my_px + ts * 2,
                                        fill='#444433', width=1)
                # Indicador de daño (grietas cuando HP bajo)
                if muro['hp'] < muro['hp_max']:
                    cx_m = mx_px + ts
                    cy_m = my_px + ts
                    self.canvas.create_line(
                        cx_m - 4, cy_m - 6, cx_m + 2, cy_m,
                        cx_m - 2, cy_m + 5,
                        fill='#FF4400', width=2
                    )
            ...
            for enemy in self.enemies:
                ...
                # Dirección del enemigo: apunta hacia el jugador
                ...
                self.tanks_dibujar_tanque(
                    ex_px, ey_px, col, dir_enemy,
                    es_boss=enemy['is_boss'], es_jugador=False
                )

            # ── Jugador ──
            px_px = self.player_pos[0] * ts + ts // 2
            py_px = self.player_pos[1] * ts + ts // 2
            # Color del jugador cambia según su HP
            pct_hp = self.player_hp / max(self.player_hp_max, 1)
            col_jugador = '#00FFCC' if pct_hp > 0.5 else ('#FFFF00' if pct_hp > 0.25 else '#FF4444')
            self.tanks_dibujar_tanque(
                px_px, py_px, col_jugador, self.player_dir,
                es_boss=False, es_jugador=True
            )
            ...
            if hasattr(self, 'lbl_enemies'):
                ...
                # Actividad 6: mostrar muros restantes junto a enemigos vivos
            ...
            # Actividad 5-B: Actualizar etiqueta y barra de HP del boss
            # Solo se muestra cuando la fase del boss esta activa.
            if hasattr(self, 'lbl_boss_hp') and hasattr(self, 'boss_phase_active'):
                ...
                if self.boss_phase_active and boss is not None:
                    # Texto numerado del HP del boss
                    self.lbl_boss_hp.config(
                        text=str(max(0, boss['hp'])) + ' / ' + str(boss['hp_max'])
                    )
                    # Barra de vida: calcula ancho proporcional al HP restante
                    if hasattr(self, 'boss_hp_bar'):
                        ...
                        # Color de la barra: morado -> rojo segun vida restante
                        col_bar = '#FF00FF' if pct_boss > 0.66 else ('#FF0088' if pct_boss > 0.33 else '#FF2222')
                        if bar_w > 0:
                            self.boss_hp_bar.create_rectangle(
                                0, 0, bar_w, 10, fill=col_bar, outline=''
                            )
                elif not self.boss_phase_active:
                    # Fase normal: mostrar puntos necesarios para invocar al boss
