    ...
    def parsear_shape(self):
        ...
        while (self.posicion < len(self.tokens) and
            self.tokens[self.posicion] in ('COLOR', 'CHANCE', 'SHAPE_TYPE',
                'ENTITY_ROLE', 'HEALTH', 'SPEED', 'FIRE_RATE', 'COLOR_BY_HEALTH')):
            ...
            # Actividad 5 - A
                ...
                # Valores validos: PLAYER, ENEMY, FINAL_BOSS
                ...
                # El lexer tokeniza cada elemento como tokens separados.
                # Leer pares (umbral_int, color_hex) hasta que el siguiente token
                # no sea '('
                cbh = {}
                while (self.posicion < len(self.tokens) and
                    ...
                    # coma separadora entre pares (opcional)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'brick_tanks' in archivo_entrada or 'tanks' in archivo_entrada:
            parser.ast['version'] = 'tanks'
        ast    = parser.parse()
        # Actividad 5-B: si el brick define ON TARGET_SCORE pero el campo
        # boss_score_threshold no fue seteado explicitamente (version anterior),
        # garantizar que el umbral por defecto (500) quede en el JSON.
        # Tambien: si el juego es TANKS y tiene FINAL_BOSS pero NO tiene
        # ON_TARGET_SCORE, el boss_score_threshold queda en 500 igualmente
        # (el runtime lo usara si el usuario sube el puntaje, aunque el evento
        # no este definido en el .brick — retrocompatibilidad hacia adelante).
        if ast.get("tipo_juego") == "TANKS":
            # El threshold ya esta en 500 por defecto desde __init__.
            # Si en el futuro el .brick define BOSS_SCORE: <numero> como token,
            # el compilador puede sobreescribirlo aqui. Por ahora se deja fijo.
            pass  # boss_score_threshold = 500 ya esta en el AST
